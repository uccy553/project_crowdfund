...
const FundCard = ({ owner, title, description, image, target, deadline, amountCollected, handleClick }) => {
    const remainingDays = daysLeft(deadline);
    
    const onClick = () => {
      handleClick({ owner, title, description, image, target, deadline, amountCollected });
    }
  return (
    <div className='sm:w-[288px] w-full rounded-[15px] bg-[#1c1c24] cursor-pointer' onClick={onClick}>
        ...
        <div className='flex flex-col p-4'>
            <div className='flex flex-row items-center mb-[18px]'>
                ...
                <p className='ml-[12px] mt-[2px] font-epilogue font-medium text-[12px] text-[#808191]'>Support</p>
            </div>

            <div className='block'>
                <h3 className='font-epilogue font-semibold text-[16px] text-white text-left leading-[26px] truncate'>{title}</h3>
                <p className='mt-[5px] font-epilogue font-normal font-[12px] text-[#808191] text-left leading-[18px] truncate'>{description}</p>
            </div>
